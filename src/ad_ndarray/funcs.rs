...
impl<S, A: LinalgScalar + Float + GradientType<A, GradientType = A>> AutoDiffable<S>
    for Dot<S, Ix1, Ix1, A>
where
    <A as GradientType<A>>::GradientType: Clone + PartialEq,
{
    ...
    fn eval_grad(
        ...
        Self::Output,
        <Self::Input as GradientType<Self::Output>>::GradientType,
    ) {
        let (a, b) = (**input).clone();
    ...
    <A as GradientType<A>>::GradientType: Clone + PartialEq,
{
    ...
    fn eval_grad(
        ...
        let grad_b = einsum("jk,i->ikj", &[&Array::eye(b.shape()[0]), &a])
            .unwrap()
    ...
    <A as GradientType<A>>::GradientType: Clone + PartialEq,
{
    ...
    fn eval_grad(
        ...
        let grad_a = einsum("ik,j->kji", &[&Array::eye(a.shape()[0]), &b])
            .unwrap()
            .into_dimensionality::<Ix3>()
            .unwrap();
    ...
    <A as GradientType<A>>::GradientType: Clone + PartialEq,
{
    ...
    fn eval_grad(
        ...
        let grad_a = einsum("il,kj->likj", &[&Array::eye(a.shape()[0]), &b])
            .unwrap()
            .into_dimensionality::<Ix4>()
            .unwrap();
        let grad_b = einsum("jm,ik->jmik", &[&Array::eye(b.shape()[0]), &a])
            .unwrap()
            .into_dimensionality::<Ix4>()
            .unwrap();
...
fn test_dot() {
    ...
    assert_eq!(
        grad_mat_mat,
        AutoTuple::from((b_mat.clone(), a_mat.clone()))
    );
}
