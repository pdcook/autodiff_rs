    ...
    <Self as Diffable<StaticArgs>>::Input: GradientType<<Self as Diffable<StaticArgs>>::Output>,
{
    ...
    fn eval_grad(
        ...
        <<Self as Diffable<StaticArgs>>::Input as GradientType<
            <Self as Diffable<StaticArgs>>::Output,
        >>::GradientType,
    );
...
pub trait ForwardDiffable<StaticArgs>: Diffable<StaticArgs> {
    ...
    /// By default, `df = df/dx * dx`. However, this can be overridden in cases where a more efficient implementation is possible (e.g. functions whose arguments and return types are arrays)
    ...
    /// NOTE: dx is the derivative of x and is not the same as dconj(x)
    ...
    fn forward_conj_grad(
        ...
        self.eval_forward_conj_grad(x, dx, static_args).1
    }
}
